...
marker = "CLIENT_MENU_2_ONLY_REFERRAL_ONE_BUTTON_20260521"
...
String? _resolveMenuPhotoUrl(
  ...
  return urls.isNotEmpty ? urls.first : null;
    ...
    print("OK: menu resolver fixed to first non-empty source only")
  ...
  Widget build(BuildContext context) {
    ...
    return ProfileFeatureShell(
      ...
      subtitle: 'Откройте персональную ссылку и отправьте её другу.',
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(
            width: double.infinity,
            child: PrimaryButton(
                text: link.isNotEmpty ? 'Открыть приглашение' : 'Подготовить приглашение',
                icon: link.isNotEmpty
                    ? Icons.open_in_new_rounded
                    : Icons.refresh_rounded,
                onTap: link.isNotEmpty
                    ? () => _openInvite(context, link)
                    : onRefresh)),
      ]),
    );
  }
    ...
    print("OK: copy button removed from referral card")

    text += f"\n// {marker}\n"
    path.write_text(text, encoding="utf-8")
    print("OK: client menu/referral UI fixed")
