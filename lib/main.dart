...
class FlowruLettersLogo extends StatelessWidget {
  ...
  const FlowruLettersLogo(
      {super.key, this.height = 24, this.fallbackColor = Colors.white});
...
class FlowInviteDeepLinks {
  ...
  static Future<String?> loadPendingInviteToken() async {
    ...
    final saved =
        (await _inviteStorage.read(key: _pendingInviteTokenKey))?.trim();
...
class FlowApi {
  ...
  Future<Map<String, dynamic>> appointmentServices(
          String token, int establishmentId) =>
      _request(
          path: '/client/appointment-services',
          token: token,
          query: {'establishment_id': '$establishmentId'});
...
class FlowNativePushBridge {
  static const MethodChannel _channel =
      MethodChannel('ru.flowru.client/push_native');
...
class _AuthScreenState extends State<AuthScreen> with TickerProviderStateMixin {
  ...
  Future<void> submitLogin() async {
    ...
    if (!_hasFullRuPhone(phone)) {
      setState(
          () => error = 'Введите полный номер телефона в формате +7XXXXXXXXXX');
      return;
    }
    ...
    try {
      ...
      await saveAndOpen(data, phone: phone, password: loginPassword.text);
  ...
  Future<void> submitRegister() async {
    ...
    try {
      ...
      await saveAndOpen(data, phone: phone, password: regPassword.text);
      TextInput.finishAutofillContext(shouldSave: true);
    } on ApiError catch (e) {
      setState(() => error = e.message);
    } catch (_) {
...
class _ClientShellState extends State<ClientShell> with WidgetsBindingObserver {
  ...
  Future<void> handleReferralLink(FlowReferralDeepLink referral) async {
    ...
    if (establishmentId <= 0 || referralCode.isEmpty || applyingReferral)
      return;
  ...
  String? get menuPhotoUrl =>
      menuPhotoUrls.isNotEmpty ? menuPhotoUrls.first : null;
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border:
                        Border.all(color: const Color(0xFFEAF1F7), width: 1.2),
                    boxShadow: [
                      BoxShadow(
      ...
      children: [
        SizedBox(
          ...
          child: Checkbox(
            ...
            side:
                BorderSide(color: kLoginInkSoft.withOpacity(0.45), width: 1.4),
            shape:
...
class _ClientPreorderScreenState extends State<ClientPreorderScreen>
  ...
  // FLOWRU_PREORDER_V23_UNIVERSAL_CART_SOFT_FOCUS_20260918
  ...
  bool get _allowed => boolValue(_settings['allowed']);

  bool get _isAppointment => _featureType == 'appointment';

  String get _appointmentSubmitLabel =>
      nonEmpty(_settings['action_label']) ?? 'Записаться';
  ...
  String get _appointmentSlotTime {
    final slot = _selectedAppointmentSlot;
    if (slot == null) return '';
    return (slot['time'] ?? '').toString().trim();
  }

  String get _submitLabel => nonEmpty(_settings['submit_label']) ?? 'Отправить';
  ...
  Future<void> _loadAppointmentServices({bool silent = false}) async {
    if (!silent && mounted) {
      setState(() {
        _loadingAppointmentServices = true;
        _error = null;
      });
    } else if (mounted) {
      setState(() {
    ...
    try {
      final res = await widget.api.appointmentServices(
        widget.token,
        widget.establishmentId,
      );

      final services = mapList(res['items']);
      Map<String, dynamic>? selected = _selectedAppointmentService;

      if (selected == null && services.isNotEmpty) {
      ...
      setState(() {
        ...
        _selectedAppointmentService = selected;
  ...
  Future<void> _loadAppointmentSlots({bool silent = false}) async {
    final serviceId = _appointmentServiceId(_selectedAppointmentService);
    if (serviceId == null) {
      if (!mounted) return;
      setState(() {
    ...
    try {
      final res = await widget.api.appointmentSlots(
        widget.token,
      ...
      final slots = mapList(res['items'])
          .where((slot) => boolValue(slot['available']))
          .toList();

      Map<String, dynamic>? selected = _selectedAppointmentSlot;
      final selectedTime = _appointmentSlotTime;

      if (selectedTime.isEmpty ||
          !slots
              .any((slot) => (slot['time'] ?? '').toString() == selectedTime)) {
      ...
      setState(() {
        ...
        _selectedAppointmentSlot = selected;
        _loadingAppointmentSlots = false;
      });
    } on ApiError catch (e) {
      if (!mounted) return;
      setState(() {
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Не удалось загрузить свободное время';
        _appointmentSlots = [];
        _selectedAppointmentSlot = null;
        _loadingAppointmentSlots = false;
      });
  ...
  Future<void> _sendAppointment() async {
    final serviceId = _appointmentServiceId(_selectedAppointmentService);
    final appointmentAt = _appointmentSlotTime;
    ...
    if (appointmentAt.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Выберите свободное время')),
      );
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Запись отправлена')),
      );
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Не удалось отправить запись';
      });
    } finally {
      if (mounted) {
        setState(() {
  ...
  String get _fidelityHeroImageUrl {
    final candidates = <String>[
      (_settings['hero_image_url'] ?? '').toString(),
      (_settings['preorder_hero_url'] ?? '').toString(),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: AnimatedBuilder(
          animation: _ambientMotion,
          builder: (context, _) {
            final wave = (math.sin(_ambientMotion.value * math.pi * 2) + 1) / 2;
            return AnimatedContainer(
              duration: const Duration(milliseconds: 380),
              curve: Curves.easeOutCubic,
              height: 92,
              decoration: BoxDecoration(
                color: _v22TimeBaseColor.withOpacity(0.92),
                borderRadius: BorderRadius.circular(32),
                border: Border.all(color: Colors.white.withOpacity(0.78)),
                boxShadow: [
                  BoxShadow(
                    color: activeColor.withOpacity(0.18 + wave * 0.08),
                    blurRadius: 30 + wave * 10,
                    spreadRadius: -4,
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 7),
                                _v25MiniOrderTrack(activeColor, wave),
                              ],
                            ),
                          ),
                          const SizedBox(width: 11),
  ...
  Widget _v25MiniOrderTrack(Color color, double wave) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final travel = math.max(0.0, constraints.maxWidth - 10).toDouble();
        return SizedBox(
          height: 9,
          child: Stack(
            alignment: Alignment.centerLeft,
            children: [
              ...
              for (final x in const [0.0, 0.5, 1.0])
                Positioned(
                  ...
                  child: Container(
                    ...
                    height: 6,
                    decoration: BoxDecoration(
                      color: x == 0 ? color : _v22TimeBaseColor,
                      shape: BoxShape.circle,
              ...
              Positioned(
                left: travel * (0.08 + wave * 0.76),
                top: wave < 0.15 ? 0.0 : 1.5,
                child: Container(
                  width: 7,
                  height: 7,
                  decoration: BoxDecoration(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Spacer(),
                        _miniMetric(
                            'Формат', _isAppointment ? 'Запись' : 'Предзаказ'),
                        const SizedBox(height: 8),
  ...
  Widget _activeCard(Map<String, dynamic> item) {
    final status = (item['status'] ?? '').toString();
    final accent = status == 'cancelled'
        ? const Color(0xFFA65E59)
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                AnimatedBuilder(
                  animation: _motion,
                  builder: (context, _) {
                    final pulse = Curves.easeInOut.transform(_motion.value);
                    return Transform.scale(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [
                            BoxShadow(
                              color: accent.withOpacity(0.22 + pulse * 0.18),
            ...
            Row(
              crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                _tracker(_statusStep(status), accent),
                const SizedBox(width: 14),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      if (createdTime.isNotEmpty || pickupTime.isNotEmpty) ...[
                        const SizedBox(height: 8),
                        Row(
                          children: [
                            Icon(
                            ...
                            Flexible(
                              child: Text(
                                createdTime.isNotEmpty && pickupTime.isNotEmpty
                                    ? '$createdTime  →  $pickupTime'
                                    : createdTime.isNotEmpty
                                        ? 'Поступил $createdTime'
                                        : 'Забрать $pickupTime',
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                                style: const TextStyle(
  ...
  Widget _tracker(int step, Color accent) {
    const labels = ['Принят', 'Готовим', 'Готов', 'Выдан'];
    final safeStep = step.clamp(1, 4).toInt();
    final icon = safeStep == 1
            ...
            children: [
              TweenAnimationBuilder<double>(
                ...
                builder: (context, wave, _) {
                  return Opacity(
                    opacity: (1 - wave).clamp(0.0, 1.0) * 0.32,
                    child: Transform.scale(
                      scale: 0.72 + wave * 0.42,
                      child: Container(
                        width: 62,
                        height: 62,
                        decoration: BoxDecoration(
                        ...
                        child: Transform.scale(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              boxShadow: [
                                BoxShadow(
                                  color: accent.withOpacity(0.28 + pulse * 0.2),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: accent.withOpacity(0.08 + pulse * 0.07),
                      blurRadius: 13,
                    ),
                  ],
                ),
              ...
              Positioned(
                ...
                child: AnimatedSwitcher(
                  duration: const Duration(milliseconds: 420),
                  child: Container(
                    key: ValueKey<String>(labels[safeStep - 1]),
                    padding: const EdgeInsets.symmetric(
                      horizontal: 7,
                      vertical: 2,
                    ),
                    decoration: BoxDecoration(
                      color: _v6Cream.withOpacity(0.94),
                      borderRadius: BorderRadius.circular(99),
      ...
      _surface(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 18),
            _selectorTitle('Время', Icons.schedule_rounded),
            const SizedBox(height: 10),
            _appointmentSlotPicker(),
            const SizedBox(height: 18),
            _selectorTitle('Комментарий', Icons.edit_note_rounded),
            const SizedBox(height: 10),
            _appointmentCommentField(),
          ],
        ),
      ),
      delay: 90,
        ...
        return Transform.translate(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(
                  color: const Color(0xFF19C7C3).withOpacity(0.20),
                  blurRadius: 24 + (t * 10),
                  offset: const Offset(0, 12),
                ),
              ],
  ...
  Widget _appointmentInfoNotice() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: FlowColors.aqua.withOpacity(0.20)),
      ),
      child: const Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Icon(Icons.verified_rounded, color: FlowColors.aqua, size: 22),
          SizedBox(width: 10),
          Expanded(
            child: Text(
              'После отправки запись появится у сотрудников. Если это время уже занято, Flowru попросит выбрать другое.',
              style: TextStyle(
                color: FlowColors.ink,
                fontSize: 12.8,
                height: 1.25,
  ...
  Widget _appointmentServicePicker() {
    if (_loadingAppointmentServices) {
      return const Center(
        child: Padding(
    ...
    return Column(
      children: [
        for (final service in _appointmentServices)
          Padding(
            padding: const EdgeInsets.only(bottom: 10),
            child: _appointmentServiceTile(service),
          ),
      ],
    );
  }

  Widget _appointmentServiceTile(Map<String, dynamic> service) {
    final id = intOrNull(service['id']);
    final selectedId = _appointmentServiceId(_selectedAppointmentService);
    final selected = id != null && id == selectedId;
    final title = (service['title'] ?? 'Услуга').toString();
    final duration = intOrNull(service['duration_minutes']) ?? 60;
    final price = toDouble(service['price']);
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected
              ? FlowColors.ink2.withOpacity(0.92)
              : FlowColors.paper2.withOpacity(0.96),
          borderRadius: BorderRadius.circular(22),
          border: Border.all(
            color:
                selected ? FlowColors.ink2 : FlowColors.ink.withOpacity(0.06),
          ),
          boxShadow: selected
              ? [
                  BoxShadow(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: selected
                    ? Colors.white.withOpacity(0.14)
                    : FlowColors.aqua.withOpacity(0.11),
                borderRadius: BorderRadius.circular(16),
              ),
              child: Icon(
                Icons.event_available_rounded,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    '$duration мин${price > 0 ? '  ${price.toStringAsFixed(0)} ₽' : ''}',
                    style: TextStyle(
                      color: selected
                          ? Colors.white.withOpacity(0.78)
                          : FlowColors.muted,
  ...
  Widget _appointmentDateChip(DateTime date) {
    final selected = _dateApi(date) == _dateApi(_appointmentDate);
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: selected
              ? const LinearGradient(colors: [FlowColors.ink2, FlowColors.aqua])
              : null,
          color: selected ? null : FlowColors.paper2.withOpacity(0.96),
          borderRadius: BorderRadius.circular(18),
          border: Border.all(
            color:
                selected ? FlowColors.aqua : FlowColors.ink.withOpacity(0.06),
          ),
        ),
        child: Text(
          _dateLabel(date),
  ...
  Widget _appointmentSlotPicker() {
    ...
    if (_appointmentSlots.isEmpty) {
      return _emptyAppointmentBox(
        ...
        text: 'Выберите другую дату или уточните время у заведения.',
      );
    }
    ...
    return Wrap(
      ...
      runSpacing: 9,
      children: [
        for (final slot in _appointmentSlots) _appointmentSlotChip(slot),
      ],
    );
  }

  Widget _appointmentSlotChip(Map<String, dynamic> slot) {
    final time = (slot['time'] ?? '').toString();
    final selected = _appointmentSlotTime == time;
  ...
  Widget _appointmentCommentField() {
    return Container(
      decoration: BoxDecoration(
  ...
  Widget _emptyAppointmentBox({
    ...
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Icon(icon, color: FlowColors.muted, size: 22),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _payment(String label, String value, IconData icon) {
