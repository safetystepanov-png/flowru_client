...
void main() async {
  ...
  try {
    ...
    firebaseInitStatus = 'firebase-init-ok-1.0.3+39';
  } catch (e) {
    final raw = e.toString();
    final safe = raw
        .replaceAll(RegExp(r'[^a-zA-Z0-9_\-]+'), '_');
    firebaseInitStatus = 'firebase-init-error-${safe.length > 90 ? safe.substring(0, 90) : safe}-1.0.3+39';
  }
...
class FlowClientPush {
  ...
  static Future<void> registerWithToken(String accessToken) async {
    ...
    try {
      await _mark(accessToken, 'debug-01-start-1.0.3+39');
      await _mark(accessToken, firebaseInitStatus);

      final messaging = FirebaseMessaging.instance;

      await _mark(accessToken, 'debug-02-before-permission-1.0.3+39');
      ...
      await _mark(
        accessToken,
        'debug-03-permission-${settings.authorizationStatus.name}-1.0.3+39',
      );

      final apnsToken = await messaging.getAPNSToken();
      if (apnsToken == null || apnsToken.trim().isEmpty) {
        await _mark(accessToken, 'debug-04-apns-empty-1.0.3+39');
      } else {
        await _mark(accessToken, 'debug-04-apns-ok-1.0.3+39');
      }

      final pushToken = await messaging.getToken();
      if (pushToken == null || pushToken.trim().isEmpty) {
        await _mark(accessToken, 'debug-05-fcm-empty-1.0.3+39');
      } else {
        await _mark(accessToken, 'debug-05-fcm-ok-1.0.3+39');
      ...
      await _mark(accessToken, 'debug-99-error-$safe-1.0.3+39');
